...
class PlayerAgent:
    ...
    def play(self, board_state: board.Board, sensor_data: Tuple, time_left: Callable):
        ...
        # 1. Handle our own previous search
        last_search_loc, last_search_result = board_state.player_search
        if last_search_result and self.tracker:
            # We caught it, rat respawns
            self.tracker = RatTracker(self.T)
        elif last_search_loc and self.tracker:
            # We missed. Explicitly set the failed location to 0% BEFORE the rat transitions
            idx = last_search_loc[1] * 8 + last_search_loc[0]
            self.tracker.belief[idx] = 0.0
            
        # 2. Handle the opponent's previous search (Free info!)
        opp_search_loc, opp_search_result = board_state.opponent_search
        if opp_search_result and self.tracker:
            # If the opponent caught it, the rat respawned. Reset our tracker.
            self.tracker = RatTracker(self.T)
        elif opp_search_loc and self.tracker:
            # The opponent missed. Zero out that cell too.
            idx = opp_search_loc[1] * 8 + opp_search_loc[0]
            self.tracker.belief[idx] = 0.0

        # Normalize the belief matrix after applying negative info
        if self.tracker and np.sum(self.tracker.belief) > 0:
            self.tracker.belief /= np.sum(self.tracker.belief)
            
        # 3. Proceed with the normal HMM update
        if self.tracker:
        ...
        # 4. Calculate Expected Value of the best possible search
        ...
        if self.tracker:
            ...
            # Only consider searching if EV is strictly positive
            if search_ev > 0.1:  
        ...
        # 7. Chance Node Resolution: The Opportunity Cost Filter
        if best_search_move and self.tracker:
            best_cell_idx = int(np.argmax(self.tracker.belief))
            p_catch = self.tracker.belief[best_cell_idx]
            
            # CONDITION 1: Absolute Certainty
            # If we are >85% sure, +4 points is almost guaranteed. Take the shot.
            if p_catch > 0.85:
                return best_search_move
                
            # CONDITION 2: Tactical Desperation
            # We calculate our current board score and compare it to the deep tree's future score.
            # If 'best_spatial_eval' is significantly worse than our current score, it means 
            # the opponent is forcing a loss of territory or points that we cannot block.
            # Only then do we gamble on a >50% rat catch to try and offset the point loss.
            current_eval = self.evaluate_board(board_state, True)
            if p_catch > 0.50 and best_spatial_eval < (current_eval - 100):
                return best_search_move
